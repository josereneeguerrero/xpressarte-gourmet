...
const platillos = [
  {
    ...
    descripcion: "Descripción del platillo. Agrega ingredientes, técnica y lo que lo hace especial.",
    precio: "L. 000",
  },
  {
    numero: "02",
...
function PlatilloCard({ p, index }: { p: (typeof platillos)[0]; index: number }) {
  ...
  return (
    ...
    >
      {/* Image placeholder */}
      <div className="relative w-full aspect-[4/3] bg-gradient-to-br from-[#1a1508] to-[#0f0f0f] flex items-center justify-center overflow-hidden">
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <h3
          className="text-[#F9FAFB] text-2xl font-semibold mb-2 leading-tight"
          style={{ fontFamily: "var(--font-playfair)" }}
        >
          {p.nombre}
        </h3>
        <p className="text-[#9CA3AF] text-sm leading-relaxed font-light flex-1 mb-5">
          {p.descripcion}
        </p>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div>
            ...
            >
              Nuestras{" "}
              <em className="not-italic text-[#E5B21A] font-semibold">Especialidades</em>
            </h2>
          </div>
          <Link
            href="/menu"
            className="shrink-0 inline-flex items-center gap-2 border border-[#E5B21A]/30 hover:border-[#E5B21A]/60 hover:bg-[#E5B21A]/5 text-[#9CA3AF] hover:text-[#E5B21A] text-sm font-medium px-7 py-3 rounded-full transition-all duration-200"
          >
            Ver carta completa →
          </Link>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {platillos.map((p, i) => (
            <PlatilloCard key={p.numero} p={p} index={i} />
          ))}
        </div>
        ...
        >
          ...
          <p className="text-[#9CA3AF]/40 text-xs mt-4">
            Disponible como QR para tu mesa · escanea y ordena
          </p>
        </motion.div>
      </div>
    </section>
